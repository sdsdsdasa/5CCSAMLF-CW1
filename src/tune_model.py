...
def dict_product(grid: dict):
    """Yield every combination from a param grid as dicts."""
    keys = list(grid.keys())
    for values in product(*[grid[k] for k in keys]):
        yield dict(zip(keys, values))

# ---------------------------- Build models list automatically (ALL combinations) ----------------------------
models = {}

# RandomForest combinations
for i, params in enumerate(dict_product(rf_grid)):
    name = f"RandomForest_{i}_{params}"
...
# GradientBoosting combinations
for i, params in enumerate(dict_product(gb_grid)):
    name = f"GradientBoosting_{i}_{params}"
...
for name, model in models.items():
    ...
    print(f"{name[:90]:90s}  R^2 (val) = {r2:.4f}")

# Rank results
results_sorted = sorted(results, key=lambda x: x[1], reverse=True)

print("\n=== Top 20 models (best -> worst) ===")
for name, r2 in results_sorted[:20]:
    print(f"{r2:.4f}  {name}")
